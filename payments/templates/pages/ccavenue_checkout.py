...
def handle_payment_response(data_dict, reference_doctype, reference_docname):
	...
	if data_dict.order_status == "Success":
		try:
			payment_request = frappe.get_doc("Payment Request", data_dict.order_id)
	...
	if order_id and frappe.db.exists("Integration Request", order_id):
		...
		if ('reference_doctype' in order_details) and ('reference_docname' in order_details):
			...
			if payment_gateway:
				...
				payment_request = frappe.get_doc("Payment Request", data.get('orderNo'))
				...
				return handle_payment_page_response(payment_request, gateway_controller, data, payment_gateway_account, kwargs=order_details, integration_request=True)

	elif data.get('orderNo'):
		payment_request = frappe.get_doc("Payment Request", data.get('orderNo'))
		request_data = frappe.db.get_value(
				payment_request.reference_doctype, payment_request.reference_name, ["company", "customer_name"], as_dict=1
			)
		...
		gateway_controller = get_gateway_controller(payment_request.doctype, payment_request.name, payment_gateway)

		return handle_payment_page_response(payment_request, gateway_controller, data, payment_gateway_account, kwargs=kwargs, integration_request=False)

	else:
		frappe.respond_as_web_page("Payment Failed", "Order ID not found", http_status_code=404)

def handle_payment_page_response(payment_request, gateway_controller, data, payment_gateway_account, kwargs=None, integration_request= False):
	if gateway_controller:
		gateway_doc = frappe.get_doc("CCAvenue Settings", gateway_controller)
		decrypted_data = gateway_doc.decrypt(data.get('encResp'), gateway_doc.get_password('working_key'))
		...
		if data_dict.get('order_status') == "Success":
			try:
				if not integration_request:
					create_request_log(kwargs, service_name="CCAvenue", name=kwargs.get("order_id", ""))
			...
			try:
				if not integration_request:
					create_request_log(kwargs, service_name="CCAvenue", name=kwargs.get("order_id", ""), error=json.dumps(data_dict, indent=4))
				frappe.db.set_value("Integration Request", data_dict.get('order_id'),{
					"status": "Failed"
				})
			except:
