...
from frappe.utils import cstr, get_url, call_hook_method
...
class CCAvenueSettings(Document):
	...
	def on_update(self):
		...
		call_hook_method(
			"payment_gateway_enabled", gateway="CCAvenue-" + self.merchant_id
		)
	...
	def get_gateway_url(self, template= False, **kwargs):
		...
		""".format(gateway_domain= gateway_domain)
	...
	def get_merchant_data(self, **kwargs):
		self.redirect_url = get_url("api/method/payments.templates.pages.ccavenue_checkout.verify_payment_status")
