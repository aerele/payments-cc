...
from frappe.utils import get_url, flt, getdate
...
def get_payment_url(data= None):
	...
	if request := frappe.db.exists("Integration Request", data.order_id):
		order_details = frappe.parse_json(
			frappe.db.get_value("Integration Request", request, "data")
		)
		order_details = frappe._dict(order_details)
		reference_doctype, reference_docname = order_details.reference_doctype,  order_details.reference_docname
		...
		if reference_doctype and reference_docname:
			...
			if payment_gateway:
				...
				if gateway_controller:
					try:
						gateway_doc = frappe.get_doc("CCAvenue Settings", gateway_controller)
						return {
							"payment_url": gateway_doc.get_payment_page_url(**order_details)
						}
					except:
						frappe.log_error("Error while getting payment url..", frappe.get_traceback())
						frappe.respond_as_web_page(_("Payment Failed"), _("Error while getting payment url"), http_status_code=500)
	else:
...
def check_already_payment_processed(request, reference_doctype, reference_docname):
	if frappe.db.get_value("Integration Request", request, "status") == "Completed":
		...
		return {
			"payment_url": get_url(redirect_url)
		}
	elif frappe.db.get_value("Integration Request", request, "status") == "Failed":
...
def handle_payment_response(data_dict, reference_doctype, reference_docname):
	...
	if data_dict.order_status == "Success":
		try:
			payemnt_request = frappe.get_doc("Payment Request", data_dict.order_id)
			payment_entry = payemnt_request.set_as_paid()

			# update reference no and date in payment entry
			frappe.db.set_value("Payment Entry", payment_entry.name, {
				"reference_no": data_dict.bank_ref_no,
				"reference_date": getdate(data_dict.trans_date)
			})
		...
		frappe.db.set_value(
			...
			{
				"status" :"Completed",
				"output": json.dumps(data_dict, indent=4)
			}
		)
		...
		try:
			payemnt_request = frappe.get_doc("Payment Request", data_dict.get('order_id'))
			payemnt_request.set_as_failed()
		except:
	...
	if order_id and frappe.db.exists("Integration Request", order_id):
		...
		if ('reference_doctype' in order_details) and ('reference_docname' in order_details):
			...
			if payment_gateway:
				...
				if gateway_controller:
					...
					handle_payment_response(data_dict, reference_doctype, reference_docname)
					
	elif data.get('orderNo'):
		payemnt_request = frappe.get_doc("Payment Request", data.get('orderNo'))
		request_data = frappe.db.get_value(
				payemnt_request.reference_doctype, payemnt_request.reference_name, ["company", "customer_name"], as_dict=1
			)
		request_data.update({"company": frappe.defaults.get_defaults().company})
		
		kwargs = {
			"amount": flt(payemnt_request.grand_total, payemnt_request.precision("grand_total")),
			"title": request_data.company,
			"description": payemnt_request.subject,
			"reference_doctype": "Payment Request",
			"reference_docname": payemnt_request.name,
			"payer_email": payemnt_request.email_to or frappe.session.user,
			"payer_name": request_data.customer_name,
			"order_id": payemnt_request.name,
			"currency": payemnt_request.currency
		}

		payment_gateway, payment_gateway_account = frappe.db.get_value("Default Gateway Account", {
			"parenttype": "Payment Request",
			"parent": payemnt_request.name,
			"gateway_settings": ["like", "%CCAvenue%"]
		}, ["payment_gateway", "payment_gateway_account"])

		gateway_controller = get_gateway_controller(payemnt_request.doctype, payemnt_request.name, payment_gateway)
		...
		if gateway_controller:
			...
			if data_dict.get('order_status') == "Success":
				try:
					create_request_log(kwargs, service_name="CCAvenue", name=kwargs.get("order_id", ""), output=json.dumps(data_dict, indent=4))
					frappe.db.set_value("Integration Request", data_dict.get('order_id'),{
						"status": "Completed"
					})
				except:
				...
				try:
					...
					frappe.db.set_value("Payment Request", data_dict.get('order_id'),{
				...
				redirect_url += "&" + urlencode({"docname": payemnt_request.name})
...
def cancel_payment():
	...
	if frappe.db.exists("Integration Request", data.get('orderNo')):
		...
		try:
			payemnt_request = frappe.get_doc("Payment Request", data_dict.get('order_id'))
			payemnt_request.set_as_cancelled()
		except:
